...
const customToTraitEnum = {
    ...
    rotationDegrees:               'action.devices.traits.Rotation',
    rotationPercent:               'action.devices.traits.Rotation',
    ...
    getStates(ids, callback, states) {
        ...
        if (!ids || !ids.length) {
            ...
            if (id === undefined) {
                ...
                //TODO: Verweise wenn Blind auf Level .3 
                if ((id.endsWith("LEVEL") && id.startsWith("hm-rpc")) || (id.endsWith("LEVEL_2") && id.startsWith("hm-rpc"))) {
                    const new_id = id.replace(".4.", ".3.");
                    this.adapter.getForeignState(new_id, (err, state) => {
                        states[id] = state && state.val;
                        setImmediate(() => this.getStates(ids, callback, states));
                    });
                    console.log(`new State HomeMatic: ${id}`);
                } else {
                            ...
                            Object.keys(attrs).forEach(attr => {
                                if (attr && attr.startsWith('get_')) {
                                    ...
                                    if (attr.substring(4).split('_').length > 1) {
                                        ...
                                        if (attr === 'get_rotationPercent' && (states[attrs[attr]] === true || states[attrs[attr]] === false)) {
                                            responseDev[dev.id][attr.substring(4)] = states[attrs[attr]] === true ? 0 : 100;
                                        } else {
...
execute(requestId, commands) {
    return new Promise(resolve => {
        if (!commands) {
            ...
            console.log('[GHOME] Invalid parameter commands - NULL');
            resolve({
        ...
        commands.forEach(command => {
            command.execution.forEach(execute => {
                ...
                console.log(`[GHOME] execute ${execute.command} => ${JSON.stringify(execute.params)}`);
                                    ...
                                    if (attrs[`set_${element.path}`]) {
                                        if (this.smartDevices[dev.id].conv2iob && this.smartDevices[dev.id].conv2iob[attrs[`set_${element.path}`]]) {
                                            try {
                                                ...
                                                console.log(`Valid convert function`);
                                            } catch (e) {
                                                ...
                                                console.log(`Invalid convert function in ${dev.id}/${attrs[`set_${element.path}`]}: ${this.smartDevices[dev.id].conv2iob[attrs[`set_${element.path}`]]}`);
                                            }
                                        }
                                        tasks.push({
